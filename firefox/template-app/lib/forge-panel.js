  ...
  _applyStyleToDocument: function _applyStyleToDocument() {
    try {
      ...
      var updateSize = function(){
        self.resize(20, 20);
        self.resize(Math.min(Math.max(doc.documentElement.scrollWidth, 20), 800), Math.min(Math.max(doc.documentElement.scrollHeight, 20), 500));
      };
      doc.documentElement.addEventListener("DOMSubtreeModified", updateSize, false);
      updateSize();
      // END FORGE ADDITION
    }
    catch(e) {
