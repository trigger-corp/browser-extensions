...
OutFile "APP_NAME_HERE-VERSION_HERE-x64.exe"
InstallDir "$$PROGRAMFILES64\APP_NAME_HERE"
...
Section 
    ...
    installation:       
        ...
        !insertmacro InstallLib REGDLL SHARED NOREBOOT_NOTPROTECTED \
            "$${BUILD_DIR64}\$${FORGE_EXECUTE_FILE64}" \
            "$$INSTDIR\$${FORGE_EXECUTE_FILE64}" \
            "$$INSTDIR"
        !insertmacro InstallLib REGDLL SHARED NOREBOOT_NOTPROTECTED \
            "$${BUILD_DIR64}\$${PRODUCT_EXECUTE_FILE64}" \
            "$$INSTDIR\$${PRODUCT_EXECUTE_FILE64}" \
            "$$INSTDIR"
            
        ; bho - x86
        ; TODO - don't install if dll >= version already exists
        !insertmacro InstallLib REGDLL SHARED NOREBOOT_NOTPROTECTED \
            "$${BUILD_DIR32}\$${FORGE_EXECUTE_FILE32}" \
            "$$INSTDIR\$${FORGE_EXECUTE_FILE32}" \
            "$$INSTDIR"
        !insertmacro InstallLib REGDLL SHARED NOREBOOT_NOTPROTECTED \
            "$${BUILD_DIR32}\$${PRODUCT_EXECUTE_FILE32}" \
            "$$INSTDIR\$${PRODUCT_EXECUTE_FILE32}" \
            "$$INSTDIR"
    ...
    ie9: 
        ...
        ; as Administrator.
        {% if defined('homepage') %}
        CreateShortCut "$$TEMP\Launcher.lnk" "$$PROGRAMFILES64\Internet Explorer\iexplore.exe" "${homepage}"
